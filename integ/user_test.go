	...
	tests := []test{
		...
		// Merging accounts
		{
			...
			VerifyFunc: func() bool {
				...
				// existingUser4
				events, err := modelsClient.GetEventsByUser(tc, &existingUser4, &models.Pagination{Size: -1})
				if err != nil {
					return false
				}
				...
				// existingUser4
				threads, err := modelsClient.GetThreadsByUser(tc, &existingUser4, &models.Pagination{})
				if err != nil {
					return false
				}
